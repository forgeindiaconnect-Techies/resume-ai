...
files.forEach(file => {
  ...
  // 1. Add lineHeight and theme to destructured props
  ...
  content = content.replace(/const lineH = [^;]+;/, "const lineH = lineHeight || 1.6;");

  // 4. Update spacingPadding to use theme.margin
  content = content.replace(/const spacingPadding = [^;]+;/, "const spacingPadding = theme?.margin ? `${theme.margin}px` : '2rem';");
